...
const tableHeader: string[] = ['S.No', 'Vendor Name', 'Phone', 'Email', 'Rejected Date', 'Actions'];
...
const RejectedVendors = () => {
    ...
    useEffect(() => {
        setLoading(true);
        (dispatch as any)(fetchVendorsByStatus('rejected' as any))
            .then((result: any) => {
                if (result?.payload?.status) {
                    const cleaned = (result.payload.data || []).map((vendor: any) => ({
                        ...
                        email: vendor?.email,
                        createdAt: vendor?.rejected_date || vendor?.createdAt
                    }));
                    setBodyData(cleaned);
                } else {
                    setBodyData([]);
    ...
    const handleView = (id: string) => {
        router.push(`/admin/vendor-rejected/view/${id}`)
    }
    ...
    return (
        <AdPageLayout
            ...
            linkHref="/admin/vendor-rejected"
            name="Vendor"
        >
            {loading ? (
                <p>Loading...</p>
